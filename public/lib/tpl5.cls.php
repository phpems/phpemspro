...
class tpl
{
	...
	{
		$limit = '/{x2;realhtml:([^}]+)}/e';
		$replace = "'<?php echo html_entity_decode(\\route::stripSlashes('.\$this->_compileArray('\${1}').')); ?>'";
		$content = preg_replace($limit,$replace,$content);
	}
	...
	public function compileRealSubstring(&$content)
	{
		...
		$replace = "'<?php echo \\\$this->G->make(\'strings\')->subString(strip_tags(html_entity_decode(\\route::stripSlashes('.\$this->_compileArray('\${1}').'))),\${2}); ?>'";
		$content = preg_replace($limit,$replace,$content);
	}
	...
	{
		$limit = '/{x2;eval:([^}]+)}/e';
		$replace = "'<?php '.\$this->_compileArray(\\route::stripSlashes('\${1}')).'; ?>'";
		$content = preg_replace($limit,$replace,$content);
	}
