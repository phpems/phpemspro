...
class tpl
{
	...
	// 添加从公共目录引入页面
	public function compileGlobaltpl(&$content)
	{
		$limit = '/{x2;globaltpl:(\w+)}/';
		//正则搜索替换，fun回调函数从$content（模板页）中匹配$limit,
		$content = preg_replace_callback($limit,function($matches){
			return "<?php \$this->_compileGlobaltpl('{$matches[1]}'); ?>";
		},$content);
	}
	public function _compileGlobaltpl($file)
	{
		if($file)$this->fetchglobaltpl($file,NULL,0);
		if($this->isCache)include 'public/data/compile/core/global/%%cpl%%'.$file.'.php';
	}
	// 
	...
	// 增加公共模板解析
	public function fetchglobaltpl($file,$par='',$type = 0,$cachename = NULL)
	{
		$this->initFile();
		$source = 'app/core/global/'.$file.'.tpl';
		$outfile = 'public/data/compile/core/global/%%cpl%%'.$file.'.php';
		if($cachename)$outcache = 'public/data/html/core/global/'.$cachename.'.html';
		else
			$outcache = 'public/data/html/core/global/'.$file.$par.'.html';
		if((!file_exists($outfile)) || (filemtime($outfile) < filemtime($source)))
		{
			$content = $this->compileTpl($source);
			files::writeFile($outfile,$content);
