{x2;if:!$_userhash}
{x2;include:header}
<div class="pages">
    {x2;endif}
    <div class="pages-tabs">
        <div class="pages-header">
            <a class="col-xs-2">
                <div class="text-center"></div>
            </a>
            <a class="col-xs-8 active">
                <div class="text-center">发现</div>
            </a>
            <a class="col-xs-2">
                <div class="text-center">
                    <i class="glyphicon glyphicon-option-horizontal"></i>
                </div>
            </a>
        </div>
        <div class="pages-content">
            <div class="pages-swiper">
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        {x2;tree:$contents[1]['data'],content,cid}
                        <div class="swiper-slide">
                            <a href="index.php?content-mobile-content&contentid={x2;v:content['contentid']}" title="{x2;v:content['contenttitle']}" class="ajax">
                                <img src="{x2;v:content['contentthumb']}" style="width:100%">
                            </a>
                        </div>
                        {x2;endif}
                    </div>
                    <div class="swiper-pagination"></div>
                </div>
            </div>
            <div class="pages-box nopadding">
                <div class="page-ele contin">
                    <a href="index.php?content-mobile-category&catid=2" class="ajax" data-page="yes" data-title="考试公告"><h4 class="bigtitle">考试公告</h4></a>
                </div>
                {x2;tree:$contents[2]['data'],content,cid}
                <div class="page-ele contin">
                    <a data-page="yes" data-title="考试公告" href="index.php?content-mobile-content&contentid={x2;v:content['contentid']}" class="ajax">
                        <img src="{x2;v:content['contentthumb']}"/>
                        <h5 class="title">
                            {x2;v:content['contenttitle']}
                        </h5>
                        <div style="clear: both">
                            <p>
                                {x2;v:content['contentintro']}
                            </p>
                            <p class="text-right">
                                {x2;v:content['contenttime']}
                            </p>
                        </div>
                    </a>
                </div>
                {x2;endif}
                <div class="page-ele contin">
                    <a data-page="yes" data-title="考试公告" href="index.php?content-mobile-category&catid=2" class="ajax">
                        <p class="text-center">更多</p>
                    </a>
                </div>
            </div>
            <div class="pages-box nopadding">
                <div class="page-ele contin">
                    <a href="index.php?content-mobile-category&catid=3" class="ajax" data-page="yes" data-title="报考指南"><h4 class="bigtitle">报考指南</h4></a>
                </div>
                {x2;tree:$contents[3]['data'],content,cid}
                <div class="page-ele contin">
                    <a data-page="yes" data-title="考试公告" href="index.php?content-mobile-content&contentid={x2;v:content['contentid']}" class="ajax">
                        <img src="{x2;v:content['contentthumb']}"/>
                        <h5 class="title">
                            {x2;v:content['contenttitle']}
                        </h5>
                        <div style="clear: both">
                            <p>
                                {x2;v:content['contentintro']}
                            </p>
                            <p class="text-right">
                                {x2;v:content['contenttime']}
                            </p>
                        </div>
                    </a>
                </div>
                {x2;endif}
                <div class="page-ele contin">
                    <a data-page="yes" data-title="考试公告" href="index.php?content-mobile-category&catid=3" class="ajax">
                        <p class="text-center">更多</p>
                    </a>
                </div>
            </div>
        </div>
        <div class="pages-footer">
            <a class="col-xs-3 active navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-record"></span><br />发现
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-play-circle"></span><br />课程
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon glyphicon-edit"></span><br />题库
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-education"></span><br />我的
                </div></a>
        </div>
    </div>
    <div class="pages-tabs">
        <div class="pages-header">
            <a class="col-xs-2">
                <div class="text-center"></div>
            </a>
            <a class="col-xs-8 active">
                <div class="text-center">课程</div>
            </a>
            <a class="col-xs-2">
                <div class="text-center">
                    <i class="glyphicon glyphicon-option-horizontal"></i>
                </div>
            </a>
        </div>
        <div class="pages-content">
            <div class="pages-box">
                <div class="page-ele contin margin">
                    <h4 class="bigtitle">最新课程</h4>
                </div>
                {x2;tree:$lessons['data'],lesson,lid}
                <div class="page-ele contin">
                    <a href="index.php?lesson-mobile-lesson&lessonid={x2;v:lesson['lessonid']}" class="ajax" data-page="yes" data-title="{x2;substring:v:lesson['lessonname'],24}">
                        <div class="col-xs-3" style="padding-top: 0.1rem;">
                            <img src="{x2;v:lesson['lessonthumb']}" class="img-circle" style="width: 86%;"/>
                        </div>
                        <div class="col-xs-9">
                            <h5 class="title">{x2;v:lesson['lessonname']}</h5>
                            <p>
                                {x2;realsubstring:v:lesson['lessonintro'],120}
                            </p>
                        </div>
                    </a>
                </div>
                {x2;endtree}
            </div>
            <div class="pages-box">
                <div class="page-ele clear">
                    {x2;tree:$lessoncats,cat,cid}
                    <div class="col-xs-6">
                        <a href="index.php?lesson-mobile-category&catid={x2;v:cat['catid']}" class="ajax" data-page="yes" data-title="{x2;v:cat['catname']}">
                            <div class="cards">
                                <div class="col-xs-4" style="padding-top: 0.15rem;">
                                    <img src="{x2;v:cat['catthumb']}" class="img-circle" style="width: 100%;"/>
                                </div>
                                <div class="col-xs-8">
                                    <h5 class="title">{x2;v:cat['catname']}</h5>
                                    <p>
                                        {x2;realsubstring:v:cat['catintro'],48}
                                    </p>
                                </div>
                            </div>
                        </a>
                    </div>
                    {x2;endtree}
                </div>
            </div>
        </div>
		<div class="pages-footer">
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-record"></span><br />发现
                </div></a>
            <a class="col-xs-3 active navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-play-circle"></span><br />课程
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon glyphicon-edit"></span><br />题库
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-education"></span><br />我的
                </div></a>
        </div>
	</div>        
    <div class="pages-tabs">
        <div class="pages-header">
            <a class="col-xs-2">
                <div class="text-center"></div>
            </a>
            <a class="col-xs-8 active">
                <div class="text-center">题库</div>
            </a>
            <a class="col-xs-2">
                <div class="text-center">
                    <i class="glyphicon glyphicon-option-horizontal"></i>
                </div>
            </a>
        </div>
        <div class="pages-content">
            <div class="pages-box">
                {x2;tree:$trainings,training,tid}
                <div class="page-ele contin margin">
                    <h4 class="bigtitle">{x2;v:training['trname']}</h4>
                </div>
                {x2;tree:$subjects[v:training['trid']],subject,sid}
                {x2;tree:$basics[v:subject['subjectid']]['data'],basic,bid}
                <div class="page-ele contin">
                    <a data-page="yes" data-title="{x2;v:basic['basic']}" href="index.php?exam-mobile-ajax-setcurrentbasic&subjectid={x2;v:subject['subjectid']}&basicid={x2;v:basic['basicid']}" class="ajax">
                        <div class="col-xs-3" style="padding-top: 0.1rem;">
                            <img src="{x2;v:basic['basicthumb']}" class="img-circle" style="width: 100%;"/>
                        </div>
                        <div class="col-xs-9">
                            <h5 class="title">{x2;v:basic['basic']}</h5>
                            <p>
                                {x2;v:basic['basicdescribe']}
                            </p>
                        </div>
                    </a>
                </div>
                {x2;endtree}
                {x2;endtree}
                {x2;endtree}
            </div>
        </div>
        <div class="pages-footer">
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-record"></span><br />发现
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-play-circle"></span><br />课程
                </div></a>
            <a class="col-xs-3 active navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon glyphicon-edit"></span><br />题库
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-education"></span><br />我的
                </div></a>
        </div>
    </div>
    <div class="pages-tabs">
        <div class="pages-header">
            <a class="col-xs-2">
                <div class="text-center"></div>
            </a>
            <a class="col-xs-8 active">
                <div class="text-center">我的</div>
            </a>
            <a class="col-xs-2">
                <div class="text-center">
                    <i class="glyphicon glyphicon-option-horizontal"></i>
                </div>
            </a>
        </div>
        <div class="pages-content">
            <div class="pages-box nopadding">
                {x2;if:$_user['userid']}
                <div class="page-ele margin">
                    <div class="col-xs-2"></div>
                    <div class="col-xs-3" style="padding: 0.1rem;">
                        <img class="img-circle" src="public/static/images/icon.jpg" style="width: 100%;"/>
                    </div>
                    <div class="col-xs-5" style="padding-top: 0.15rem;">
                        <h5 class="title">{x2;if:$_user['userrealname']}{x2;$_user['userrealname']}{x2;else}未设置{x2;endif}</h5>
                        <p>
                            账号：{x2;$_user['username']}
                        </p>
                    </div>
                    <div class="col-xs-2"></div>
                </div>
                {x2;else}
                <div class="page-ele margin">
                    <div class="col-xs-4"></div>
                    <div class="col-xs-4 text-center" style="padding: 0.1rem;">
                        <a class="btn btn-primary btn-block ajax" href="index.php?user-mobile-login">登录/注册</a>
                    </div>
                    <div class="col-xs-4"></div>
                </div>
                {x2;endif}
                <div class="page-ele">
                    <ul class="listmenu">
                        <li><a href="index.php?lesson-mobile" class="ajax" data-page="yes" data-title="我的课程"><i class="glyphicon glyphicon-play-circle"></i> 我的课程 <i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                        <li><a href="index.php?exam-mobile" class="ajax" data-page="yes" data-title="我的题库"><i class="glyphicon glyphicon-edit"></i> 我的考场 <i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                        <li><a href="index.php?user-mobile-orders" class="ajax"><i class="glyphicon glyphicon-credit-card"></i> 我的订单 <i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                        <li><a href="index.php?user-mobile-user-info" class="ajax"><i class="glyphicon glyphicon-user"></i> 个人信息 <i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                        <li><a href="index.php?user-mobile-user-password" class="ajax"><i class="glyphicon glyphicon-cog"></i> 修改密码 <i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                    </ul>
                </div>
                <div class="page-ele hide">
                    <ul class="listmenu">
                        <li><a href="">推荐「毕过」给好友 <span class="text-danger" style="font-size: 0.14rem;margin-left: 0.24rem;">邀请可得5元优惠券</span><i class="glyphicon glyphicon-menu-right pull-right"></i></a></li>
                    </ul>
                </div>
                {x2;if:$_user['userid']}
                <div class="page-ele clear">
                    <p class="col-xs-12">
                        <a class="btn btn-primary btn-block ajax" href="index.php?user-mobile-login-logout">退出登录</a>
                    </p>
                </div>
                {x2;endif}
            </div>
        </div>
        <div class="pages-footer">
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-record"></span><br />发现
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-play-circle"></span><br />课程
                </div></a>
            <a class="col-xs-3 navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon glyphicon-edit"></span><br />题库
                </div></a>
            <a class="col-xs-3 active navibutton"><div class="text-center">
                    <span class="glyphicon glyphicon-education"></span><br />我的
                </div></a>
        </div>
    </div>
    <script>
        $(function(){
            var mySwiper = new Swiper('.swiper-container',{
                "pagination":".swiper-pagination",
                "loop":false,
                "autoplay": 0,
                "observer":true,
                "observeParents":true
            });
            localStorage.removeItem("prepages");
        });
    </script>
    {x2;if:!$_userhash}
</div>
{x2;include:footer}
{x2;endif}