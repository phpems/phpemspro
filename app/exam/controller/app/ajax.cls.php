...
class ajax
{
    ...
    public function cancelfavor()
    {
        ...
        $username = \exam\app::$_user['sessionusername'];
        $subject = \exam\model\points::getSubjectById(\exam\app::$_user['currentsubject']);
        ...
        if($question['questionparent'])
        {
            ...
            {
                $number = favor::getFavorNumberByPointid($subject['subjectdb'],\exam\app::$_user['sessionusername'],$p);
                \pedis::getInstance()->setHashData('favornumber',\exam\app::$_user['sessionusername'].'-'.$subject['subjectdb'].'-'.$p,$number);
            }
        }
        else
        {
